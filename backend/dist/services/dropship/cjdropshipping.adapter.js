...
async function cjFetch(path, opts = {}) {
    ...
    if (!res.ok)
        throw new Error(`CJ API ${res.status}`);
    return res.json();
}
export const cjAdapter = {
    ...
    async searchProducts(query, page = 1) {
        if (!env.CJ_API_TOKEN)
            return mockCjSearch(query);
        const res = await cjFetch(`/product/list?productName=${encodeURIComponent(query)}&pageNum=${page}&pageSize=20`);
    ...
    async getProduct(externalId) {
        if (!env.CJ_API_TOKEN)
            return mockCjGet(externalId);
        const res = await cjFetch(`/product/query?pid=${externalId}`);
        const p = res.data;
        return {
            externalId: p.pid, externalUrl: `https://cjdropshipping.com/product/${p.productSku}.html`,
    ...
    async placeOrder(req) {
        if (!env.CJ_API_TOKEN)
            return { externalOrderId: 'CJ-MOCK-' + Date.now(), status: 'placed', totalCents: 0 };
    ...
    async getOrderStatus(externalOrderId) {
        if (!env.CJ_API_TOKEN)
            return { status: 'shipped', trackingNumber: 'TRACK' + externalOrderId };
        const res = await cjFetch(`/logistic/trackInfo?trackNumber=${externalOrderId}`);
        return { status: res.data?.statusName ?? 'pending', trackingNumber: res.data?.trackingNumber, carrier: res.data?.logisticsName };
    },
};
function mockCjSearch(q) {
    return [{
