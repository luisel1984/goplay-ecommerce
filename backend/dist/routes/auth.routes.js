...
router.post('/signup', async (req, res, next) => {
    try {
        ...
        if (exists)
            throw new HttpError(409, 'Cet email est déjà utilisé', 'email_taken');
...
router.post('/refresh', async (req, res, next) => {
    try {
        ...
        if (!refreshToken)
            throw new HttpError(400, 'refreshToken required');
...
// Helpers
function sanitize(u) { const { passwordHash, ...safe } = u; return safe; }
