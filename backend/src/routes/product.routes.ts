...
router.get('/', async (req, res, next) => {
  try {
    ...
    const where: any = { active: true, publishedAt: { not: null } };
    if (q.q) where.OR = [
    ...
    if (q.minPrice) where.priceCents = { ...where.priceCents, gte: q.minPrice * 100 };
    if (q.maxPrice) where.priceCents = { ...where.priceCents, lte: q.maxPrice * 100 };
    if (q.inStock) where.stock = { gt: 0 };

    const orderBy: any =
      q.sort === 'price_asc' ? { priceCents: 'asc' } :
...
function toPublic(p: any, full = false) {
  const data: any = {
  ...
  if (full) {
    ...
    data.images = (p.images ?? []).map((i: any) => ({ url: i.url, alt: i.alt }));
    data.variants = (p.variants ?? []).map((v: any) => ({
      id: v.id, sku: v.sku, name: v.name, attributes: v.attributes,
      price: (v.priceCents ?? p.priceCents) / 100, stock: v.stock,
    }));
    data.reviews = (p.reviews ?? []).map((r: any) => ({
