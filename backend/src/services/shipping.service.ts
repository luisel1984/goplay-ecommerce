 ...
 * ═══════════════════════════════════════════════════════════
 * COLISSIMO (La Poste)
 * Doc: https://www.colissimo.entreprise.laposte.fr/system/files/imagecontent/docs/spec_ws_affranchissement.pdf
 * ═══════════════════════════════════════════════════════════
 */
export interface ColissimoLabelInput {
  recipient: {
  ...
  weightGrams: number;
  productCode?: 'DOM' | 'DOS' | 'COL' | 'CDS'; // DOM=domicile, DOS=signé, etc.
  orderNumber: string;
}
...
export async function createColissimoLabel(input: ColissimoLabelInput): Promise<ShippingLabel> {
  ...
  if (!res.ok) throw new Error(`Colissimo API: ${res.status}`);
  const json = await res.json() as any;
  const trackingNumber = json.labelV2Response?.parcelNumber as string;
...
/**
 * ═══════════════════════════════════════════════════════════
 * MONDIAL RELAY (puntos relay)
 * Doc: https://www.mondialrelay.fr/solutionspro/documentation-technique/
 * ═══════════════════════════════════════════════════════════
 */
 ...
 */
export async function findMondialRelayPoints(postalCode: string, country = 'FR', limit = 7): Promise<MondialRelayPoint[]> {
  const params = {
...
function signMondialRelay(params: Record<string, string>): string {
...
/**
 * ═══════════════════════════════════════════════════════════
 * Tarifas — calcula coste de envío según peso, método y destino
 * ═══════════════════════════════════════════════════════════
 */
export interface ShippingQuote {
  method: 'colissimo' | 'mondial-relay' | 'chronopost' | 'pickup';
  label: string;
  priceCents: number;
  estimatedDays: string;
}

export function getShippingQuotes(opts: { country: string; weightGrams: number; subtotalCents: number }): ShippingQuote[] {
  ...
  if (country === 'FR') {
    // Mondial Relay
    let mr = 320;
    if (weightGrams > 500)  mr = 450;
    if (weightGrams > 2000) mr = 590;
    ...
    // Colissimo
    let coli = 455;
    if (weightGrams > 500)  coli = 605;
    if (weightGrams > 1000) coli = 740;
    if (weightGrams > 2000) coli = 950;
