...
import type { DropshipAdapter, DropshipProduct } from './types.js';
...
export function getAdapter(source: ProductSource): DropshipAdapter | null {
...
export async function importDropshipProduct(opts: {
  source: ProductSource; externalId: string;
  markupPercent?: number;     // ej. 150 → precio = costo * 2.5
  categoryId?: string; supplierId?: string;
}) {
  const adapter = getAdapter(opts.source);
  if (!adapter) throw new Error(`No adapter for ${opts.source}`);

  const dp: DropshipProduct = await adapter.getProduct(opts.externalId);
    ...
    create: {
      ...
      variants: dp.variants ? {
        create: dp.variants.map(v => ({
          sku: `${sku}-${v.externalId}`, name: v.name, attributes: v.attributes as any,
...
export async function syncAllStock() {
  for (const source of ['ALIEXPRESS', 'CJDROPSHIPPING'] as const) {
    const adapter = adapters[source];
    if (!adapter) continue;

    const products = await prisma.product.findMany({
      where: { source: source as ProductSource, externalId: { not: null }, active: true },
...
export async function forwardOrderToSuppliers(orderId: string) {
  ...
  for (const item of order.items) {
    if (item.product.source === 'MANUAL' || !item.product.supplierId) continue;
