...
function signTopRequest(params: Record<string, string>): string {
...
async function callTop<T = any>(method: string, params: Record<string, any>): Promise<T> {
...
export const aliexpressAdapter: DropshipAdapter = {
  ...
  async searchProducts(query, page = 1) {
    if (!env.ALIEXPRESS_APP_KEY) return mockSearchAli(query);
    const res = await callTop<any>('aliexpress.ds.product.search', { keywords: query, page_size: 20, page_no: page });
  ...
  async getProduct(externalId) {
    if (!env.ALIEXPRESS_APP_KEY) return mockGetAli(externalId);
    const res = await callTop<any>('aliexpress.ds.product.get', { product_id: externalId, ship_to_country: 'FR', target_currency: 'EUR', target_language: 'FR' });
  ...
  async placeOrder(req: DropshipOrderRequest): Promise<DropshipOrderResponse> {
    ...
    const res = await callTop<any>('aliexpress.ds.order.create', {
  ...
  async getOrderStatus(externalOrderId) {
    if (!env.ALIEXPRESS_APP_KEY) return { status: 'shipped', trackingNumber: 'TRACK' + externalOrderId, carrier: 'AliExpress Standard' };
    const res = await callTop<any>('aliexpress.ds.order.tracking.get', { ae_order_id: externalOrderId });
...
function parseAliAttrs(s: string): Record<string, string> {
  const out: Record<string, string> = {};
  s.split(';').forEach(part => { const [k, v] = part.split(':'); if (k && v) out[k] = v; });
  return out;
}

// ─── Mock para development sin credenciales ─────────────────
function mockSearchAli(q: string): DropshipProduct[] {
  return [{
...
function mockGetAli(id: string): DropshipProduct {
